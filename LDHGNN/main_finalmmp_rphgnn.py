...
from rphgnn.datasets.load_finalmmp_data import  load_dgl_data
...
reset_seed(seed)
print("seed = ", seed)
f_log=open('./txtlog/'+'original'+str(args.seed),'a')
...
if running_leaderboard_mag:
    ...
    model_save_path = os.path.join(model_save_dir, "finalmmp_mag_seed_{}.pt".format(seed))
else:
...
def train_and_eval():
    ...
    if running_leaderboard_mag:
        ...
        with torch.no_grad():
            ...
            f_log.write(f"Results of OGB Evaluator: valid_acc = {ogb_valid_acc}, test_acc = {ogb_test_acc}\n")
            f_log.flush()
            ...
            torch.save(train_emb,'train_emb_finalmmp.pt')
