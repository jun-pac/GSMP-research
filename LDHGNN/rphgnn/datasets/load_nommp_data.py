...
def load_mag(device):
    ...
    for etype, adj in zip(etypes, adjs):
        ...
        if stype == dtype:
            if not (os.path.exists('../CLGNN/graph/nommp_src.npy') and os.path.exists('../CLGNN/graph/nommp_dst.npy')):
                newsrc=[]
                newdst=[]
                ...
                np.save('../CLGNN/graph/nommp_src',src)
                np.save('../CLGNN/graph/nommp_dst',dst)
                #new_edges[(stype, 'r'+rtype, dtype)] = (np.concatenate((src, newsrc)), np.concatenate((dst, newdst)))
            else:
                src=np.load('../CLGNN/graph/nommp_src.npy')
                dst=np.load('../CLGNN/graph/nommp_dst.npy')
