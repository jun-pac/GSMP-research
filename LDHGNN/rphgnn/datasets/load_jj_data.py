...
def load_mag(device):
    ...
    for etype, adj in zip(etypes, adjs):
        ...
        src = src.numpy()
        dst = dst.numpy()
        print(f"cur real path {os.path.realpath(__file__)}")

        if stype == dtype:
            if not (os.path.exists('../CLGNN/graph/pmp_src.npy') and os.path.exists('../CLGNN/graph/pmp_dst.npy')):
                newsrc=[]
                newdst=[]
                ...
                src,dst=np.concatenate((src, newsrc)),np.concatenate((dst, newdst)) 
                np.save('../CLGNN/graph/pmp_src',src)
                np.save('../CLGNN/graph/pmp_dst',dst)
                #new_edges[(stype, 'r'+rtype, dtype)] = (np.concatenate((src, newsrc)), np.concatenate((dst, newdst)))
            else:
                src=np.load('../CLGNN/graph/pmp_src.npy')
                dst=np.load('../CLGNN/graph/pmp_dst.npy')
    ...
    return new_g, target_node_type, feature_node_types, labels, n_classes, train_nid, val_nid, test_nid, paper_year


def load_dgl_mag(embedding_size):
    device = "cpu"
    
    g, target_node_type, feature_node_types, labels, n_classes, train_index, valid_index, test_index, paper_year = load_mag(device)
    ...
    return g, target_node_type, feature_node_types, (train_index, valid_index, test_index), paper_year
...
def nrl_update_features(dataset, hetero_graph, excluded_ntypes, 
    ...
    nrl_cache_path = os.path.join("./cache/{}_jj.p".format(dataset))
...
def load_dgl_data(dataset, use_all_feat=False, embedding_size=None, use_nrl=False, random_state=None):
    ...
    if dataset == "mag":
        hetero_graph, target_node_type, feature_node_types, (train_index, valid_index, test_index), paper_year = load_dgl_mag(embedding_size=embedding_size)        
    ...
    return hetero_graph, target_node_type, feature_node_types, (train_index, valid_index, test_index), \
           batch_size, num_epochs, patience, validation_freq, convert_to_tensor, paper_year
