...
def load_mag(device):
    ...
    for etype, adj in zip(etypes, adjs):
        ...
        if stype == dtype:
            if not (os.path.exists('../CLGNN/graph/finalmmp_src.npy') and os.path.exists('../CLGNN/graph/finalmmp_dst.npy')):
                newsrc=[]
                newdst=[]
                ...
                np.save('../CLGNN/graph/finalmmp_src',src)
                np.save('../CLGNN/graph/finalmmp_dst',dst)
            else:
                src=np.load('../CLGNN/graph/finalmmp_src.npy')
                dst=np.load('../CLGNN/graph/finalmmp_dst.npy')
