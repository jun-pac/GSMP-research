...
def compute_edge_weights(start_idx, end_idx, paper_year, src, dst):
    partial_edge_weight = torch.zeros(end_idx - start_idx)

    for idx in range(start_idx, end_idx):
        ys = min(2023, paper_year[src[idx]])
        yd = min(2023, paper_year[dst[idx]])
...
def parallel_compute(edge_weight, paper_year, src, dst, num_workers):
    pool = multiprocessing.Pool(processes=num_workers)
    chunk_size = len(src) // num_workers
    jobs = []

    for i in range(num_workers):
        start_idx = i * chunk_size
        end_idx = len(src) if i == num_workers - 1 else (i + 1) * chunk_size 
        jobs.append(pool.apply_async(compute_edge_weights, (start_idx, end_idx, paper_year, src, dst)))
...
def prepare_label_emb(args, g, labels, n_classes, train_idx, valid_idx, test_idx, paper_year, label_teacher_emb=None):
    ...
    if not os.path.isfile("./dgl_heize_edge_weight_rev.pt"):
        increase_file_limit(4096)
        src,dst=g.edges()
        edge_weight=torch.zeros(len(src))
        print(f"len edge_weight: {edge_weight.shape}")
        print(f"edge_weight.dtype: {edge_weight.dtype}")
        
        parallel_compute(edge_weight, paper_year, src, dst, num_workers=48)
        torch.save(edge_weight,"./dgl_heize_edge_weight_rev.pt")
    else:
        edge_weight=torch.load("./dgl_heize_edge_weight_rev.pt")

    
    t0=time.time()
    for hop in range(args.label_num_hops):
        print(f"Compute {hop+1}th neighbor-averaged labels... {time.time()-t0:.4f}")
        y = heize_neighbor_average_labels(g, y.to(torch.float), edge_weight, args)
...
def heize_neighbor_average_labels(g, feat, edge_weight, args):
    """
