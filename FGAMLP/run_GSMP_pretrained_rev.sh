...
gpu=0
output_emb_prefix=/fs/ess/PAS1289/GAMLP/ogbn-papers100M-gsmp_rev_pretrained.node-emb

if [ ! -f "${output_emb_prefix}_${num_hops}.pt" ]; then
    python -u ./data/preprocess_GSMP_rev_papers100m.py \
        --root ${DATA_ROOT_DIR} \
        --num_hops ${num_hops} \
        --pretrained_emb_path ${input_emb_path} \
        --output_emb_prefix ${output_emb_prefix} \
        |& tee ./gamlp.ogbn-papers100M.prepare-GSMP_rev_pretrained.log
fi
python -u main_GSMP_rev.py \
    --seed 1 \
    --gpu ${gpu} \
    --num-runs ${NUM_RUNS} \
    --root ${DATA_ROOT_DIR} \
    ...
    --bns \
    --temp 0.001 \
    |& tee ./gamlp_GSMP_rev.ogbn-papers100M._pretrained.log