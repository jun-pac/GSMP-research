...
gpu=0
output_emb_prefix=/fs/ess/PAS1289/GAMLP/ogbn-papers100M-heize_pretrained.node-emb

if [ ! -f "${output_emb_prefix}_${num_hops}.pt" ]; then
    python -u ./data/preprocess_heize_papers100m.py \
        --root ${DATA_ROOT_DIR} \
        --num_hops ${num_hops} \
        --pretrained_emb_path ${input_emb_path} \
        --output_emb_prefix ${output_emb_prefix} \
        |& tee ./gamlp.ogbn-papers100M.prepare-heize_pretrained10.log
fi
python -u main_heize_rev.py \
    ...
    --bns \
    --temp 0.001 \
    |& tee ./gamlp.ogbn-papers100M.heize_mixed_pretrained10.log