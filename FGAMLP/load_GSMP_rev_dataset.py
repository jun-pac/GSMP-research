...
def compute_local_edge_weights_chunk(args):
    g, paper_year, dst_start, dst_end = args
    import numpy as np
    adj_csc = g.adj(scipy_fmt='csc')
    edge_weights = np.zeros(adj_csc.indptr[dst_end] - adj_csc.indptr[dst_start], dtype=np.float32)
    offset = 0
    for dst in range(dst_start, dst_end):
        start = adj_csc.indptr[dst]
        end = adj_csc.indptr[dst + 1]
        srcs = adj_csc.indices[start:end]  # in-neighbors of dst
        if len(srcs) == 0:
            continue
        src_years = paper_year[srcs].cpu().numpy()
...
def compute_local_edge_weights_parallel(g, paper_year, num_workers=48):
    ...
    for i in range(num_workers):
        ...
        if dst_start >= dst_end:
            continue
        args_list.append((g, paper_year, dst_start, dst_end))
...
def prepare_label_emb(args, g, labels, n_classes, train_idx, valid_idx, test_idx, paper_year, label_teacher_emb=None):
    ...
    if not os.path.isfile("./dgl_gsmp_rev_edge_weight.pt"):
        ...
        torch.save(edge_weight,"./dgl_gsmp_rev_edge_weight.pt")
    else:
        edge_weight=torch.load("./dgl_gsmp_rev_edge_weight.pt")
