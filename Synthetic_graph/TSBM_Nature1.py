...
if __name__ == "__main__":
    ...
    f_log0=open('./txtlog/'+'NA_'+'K'+str(args.K)+'_'+'G'+str(args.G)+'_'+'s'+str(args.feat_std)+'_0','a')
    f_log1=open('./txtlog/'+'NA_'+'K'+str(args.K)+'_'+'G'+str(args.G)+'_'+'s'+str(args.feat_std)+'_1','a')
    # f_log2=open('./txtlog/'+'NA_'+'K'+str(args.K)+'_'+'G'+str(args.G)+'_'+'s'+str(args.feat_std)+'_2','a')
    f_logm=open('./txtlog/'+'NA_matrix_'+'K'+str(args.K)+'_'+'G'+str(args.G)+'_'+'s'+str(args.feat_std),'a')

    f_log0.write('NA_'+'K'+str(args.K)+'_'+'G'+str(args.G)+'_'+'s'+str(args.feat_std)+'_0\n')
    f_log0.flush()
    f_log1.write('NA_'+'K'+str(args.K)+'_'+'G'+str(args.G)+'_'+'s'+str(args.feat_std)+'_1\n')
    f_log1.flush()
    # f_log2.write('NA_'+'K'+str(args.K)+'_'+'G'+str(args.G)+'_'+'s'+str(args.feat_std)+'_2\n')
    # f_log2.flush()
    ...
    for i in range(args.repeat):
        for num in range(9):
            ...
            for y in range(args.num_label):
                alpha=args.K*(8-num)/8
                beta=args.K*3/2*num/8
                for t in range(args.num_time):
                    prob[y][y][t]=alpha+beta*(t/args.num_time)
                    # prob[y][y][t]=args.K
            ...
            for y1 in range(args.num_label):
                for y2 in range(y1):
                    ...
                    beta=mx*3/2*num/8
                    for t in range(args.num_time):
                        prob[y1][y2][t]=alpha+beta*(t/args.num_time)
                        prob[y2][y1][t]=prob[y1][y2][t]
            f_logm.write(f"args.K : {args.K}, args.G : {args.G}, num : {num}\n")
            f_logm.flush()
            for t in range(args.num_time):
                for y1 in range(args.num_label):
                    for y2 in range(args.num_label):
                        f_logm.write(f"{prob[y1][y2][t]:.4f} ")
                    f_logm.write("\n")
                f_logm.write("\n")
                f_logm.flush()
...
conda activate vv
# 0426
python TSBM_Nature1.py --K 0.4 --G 0.8 --epoch 600
python TSBM_Nature1.py --K 0.4 --G 0.6 --epoch 600
python TSBM_Nature1.py --K 0.4 --G 0.4 --epoch 600
python TSBM_Nature1.py --K 0.4 --G 0.3 --epoch 600
python TSBM_Nature1.py --K 0.2 --G 0.8 --epoch 600
python TSBM_Nature1.py --K 0.2 --G 0.6 --epoch 600
python TSBM_Nature1.py --K 0.2 --G 0.4 --epoch 600
python TSBM_Nature1.py --K 0.2 --G 0.3 --epoch 600
'''