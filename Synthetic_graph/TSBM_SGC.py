...
from SGCmodule import SGConv, SGConv_JJ, SGConv_PNY
...
def train_base(g, f_log):
    ...
    model = SGConv(features.shape[1], args.num_label, k=2, cached=True, bias=True).to(device)
...
def train_JJ(g, f_log, times):
    ...
    labels = g.ndata["label"]
    masks = g.ndata["train_mask"].type(torch.bool), g.ndata["val_mask"].type(torch.bool), g.ndata["test_mask"].type(torch.bool)
    model = SGConv_JJ(labels, times, args.num_time, args.num_label, args.split, features.shape[1], args.num_label, k=2, cached=True, bias=True).to(device)
    train(g, features, labels, masks, model, f_log)
    acc = evaluate(g, features, labels, masks[2], model)
    # print(f"{acc:.4f}", end=' ', flush=True)
    f_log.write(f"{acc:.4f}\n")
...
def train_PNY(g, f_log, relative_conn, times):
    ...
    labels = g.ndata["label"]
    masks = g.ndata["train_mask"].type(torch.bool), g.ndata["val_mask"].type(torch.bool), g.ndata["test_mask"].type(torch.bool)
    model = SGConv_PNY(labels, times, args.num_time, args.num_label, args.split, relative_conn, features.shape[1], args.num_label, k=2, cached=True, bias=True).to(device)
    train(g, features, labels, masks, model, f_log)
    acc = evaluate(g, features, labels, masks[2], model)
    # print(f"{acc:.4f}", end=' ', flush=True)
    f_log.write(f"{acc:.4f}\n")
...
if __name__ == "__main__":
    ...
    f_log_base=open('./txtlog/'+'SGC_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_b','a')
    f_log_MMP=open('./txtlog/'+'SGC_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_mmp','a')
    f_log_PMP=open('./txtlog/'+'SGC_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pmp','a')
    f_log_JJ=open('./txtlog/'+'SGC_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_jj','a')
    f_log_PNY=open('./txtlog/'+'SGC_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pny','a')

    # f_log_base.write('SGC_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_b'+'\n')
    # f_log_base.flush()
    # f_log_MMP.write('SGC_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_mmp'+'\n')
    # f_log_MMP.flush()
    # f_log_PMP.write('SGC_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pmp'+'\n')
    # f_log_PMP.flush()
    # f_log_JJ.write('SGC_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_jj'+'\n')
    # f_log_JJ.flush()
    # f_log_PNY.write('SGC_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pny'+'\n')
    # f_log_PNY.flush()    
    ...
    for i in range(args.repeat):
        ...
        if args.L==0:
            ...
            for y1 in range(args.num_label):
                for y2 in range(y1):
                    decay_matrix[y1][y2]=random.uniform(0.3,0.9)
                    decay_matrix[y2][y1]=decay_matrix[y1][y2]
            ...
            for y1 in range(args.num_label):
                for y2 in range(y1):
                    decay_matrix[y1][y2]=random.uniform(0,1)
                    decay_matrix[y2][y1]=decay_matrix[y1][y2]
...
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.4 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.5 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.6 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.7 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.45 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.55 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.65 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.75 --epoch 200; done

while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.8 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.85 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.9 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.95 --epoch 200; done
...
while sleep 1; do python TSBM_SGC.py --L 6 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 7 --epoch 200; done

while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.35 --epoch 200; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.3 --epoch 200; done



# 0517
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.3 --epoch 400; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.4 --epoch 400; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.5 --epoch 400; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.6 --epoch 400; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.7 --epoch 400; done
while sleep 1; do python TSBM_SGC.py --L 0 --decay 0.8 --epoch 400; done
