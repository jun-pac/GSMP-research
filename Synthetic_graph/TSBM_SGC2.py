...
from SGC2module import SGC2onv, SGC2onv_JJ, SGC2onv_PNY
...
def train_base(g, f_log):
    ...
    model = SGC2onv(features.shape[1], 16, args.num_label, k=2, cached=True, bias=True).to(device)
...
def train_JJ(g, f_log, times):
    ...
    labels = g.ndata["label"]
    masks = g.ndata["train_mask"].type(torch.bool), g.ndata["val_mask"].type(torch.bool), g.ndata["test_mask"].type(torch.bool)
    model = SGC2onv_JJ(labels, times, args.num_time, args.num_label, args.split, features.shape[1], 16,args.num_label, k=2, cached=True, bias=True).to(device)
    train(g, features, labels, masks, model, f_log)
    acc = evaluate(g, features, labels, masks[2], model)
    # print(f"{acc:.4f}", end=' ', flush=True)
    f_log.write(f"{acc:.4f}\n")
...
def train_PNY(g, f_log, relative_conn, times):
    ...
    labels = g.ndata["label"]
    masks = g.ndata["train_mask"].type(torch.bool), g.ndata["val_mask"].type(torch.bool), g.ndata["test_mask"].type(torch.bool)
    model = SGC2onv_PNY(labels, times, args.num_time, args.num_label, args.split, relative_conn, features.shape[1], 16,args.num_label, k=2, cached=True, bias=True).to(device)
    train(g, features, labels, masks, model, f_log)
    acc = evaluate(g, features, labels, masks[2], model)
    # print(f"{acc:.4f}", end=' ', flush=True)
    f_log.write(f"{acc:.4f}\n")
...
if __name__ == "__main__":
    ...
    # f_log_base=open('./txtlog/'+'SGC2_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_b','a')
    # f_log_MMP=open('./txtlog/'+'SGC2_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_mmp','a')
    f_log_PMP=open('./txtlog/'+'SGC2_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pmp','a')
    # f_log_JJ=open('./txtlog/'+'SGC2_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_jj','a')
    f_log_PNY=open('./txtlog/'+'SGC2_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pny','a')
    # f_log_base.write('SGC2_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_b'+'\n')
    # f_log_base.flush()
    # f_log_MMP.write('SGC2_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_mmp'+'\n')
    # f_log_MMP.flush()
    # f_log_PMP.write('SGC2_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pmp'+'\n')
    # f_log_PMP.flush()
    # f_log_JJ.write('SGC2_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_jj'+'\n')
    # f_log_JJ.flush()
    # f_log_PNY.write('SGC2_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pny'+'\n')
    # f_log_PNY.flush()    
    ...
    for i in range(args.repeat):
        ...
        if args.L==0:
            ...
            for y in range(args.num_label):
                prob[y][y][0]=random.uniform(0, args.K)   
            for y1 in range(args.num_label):
                for y2 in range(y1):
                    prob[y1][y2][0]=random.uniform(0, args.K * args.G)
                    prob[y2][y1][0]=prob[y1][y2][0]
            for t in range(1,args.num_time):
                for y1 in range(args.num_label):
                    for y2 in range(args.num_label):
            ...
            for y1 in range(args.num_label):
                for y2 in range(y1):
                    prob[y1][y2][0]=random.uniform(0,torch.sqrt(prob[y1][y1][0]*prob[y2][y2][0])*args.G)
                    prob[y2][y1][0]=prob[y1][y2][0]
            for t in range(1,args.num_time):
                for y1 in range(args.num_label):
                    for y2 in range(args.num_label):
            ...
            for y1 in range(args.num_label):
                for y2 in range(y1):
                    decay_matrix[y1][y2]=random.uniform(0.55,0.75)
            ...
            for y1 in range(args.num_label):
                for y2 in range(y1):
                    decay_matrix[y1][y2]=random.uniform(0.4,0.6)
                    decay_matrix[y2][y1]=decay_matrix[y1][y2]
            ...
            for y1 in range(args.num_label):
                for y2 in range(y1):
                    decay_matrix[y1][y2]=random.uniform(0.8,1)
                    decay_matrix[y2][y1]=decay_matrix[y1][y2]
            ...
            for y1 in range(args.num_label):
                for y2 in range(y1):
                    decay_matrix[y1][y2]=random.uniform(0.7,0.9)
                    decay_matrix[y2][y1]=decay_matrix[y1][y2]
        ...
        feat_std=[args.feat_std]*args.num_label
        for i in range(total_node):
            cur_label = label[i]
        ...
        # t0=time.time()
        # acc_PNY=train_PNY(sym_g, f_log_trash, relative_conn, times)
        # print(f"PNY : {time.time()-t0}")
        # f_log_PNY.write(f"{time.time()-t0}\n")
        # f_log_PNY.flush()

        # t0=time.time()
        # acc_base=train_base(new_g, f_log_trash)
        # print(f"Base : {time.time()-t0}")
        # f_log_base.write(f"{time.time()-t0}\n")
        # f_log_base.flush()

        # t0=time.time()
        # acc_PMP=train_PMP(sym_g, f_log_trash)
        # print(f"PMP : {time.time()-t0}")
        # f_log_PMP.write(f"{time.time()-t0}\n")
        # f_log_PMP.flush()

        # t0=time.time()
        # acc_MMP=train_MMP(mono_g, f_log_trash)
        # print(f"MMP : {time.time()-t0}")
        # f_log_MMP.write(f"{time.time()-t0}\n")
        # f_log_MMP.flush()

        # t0=time.time()
        # acc_JJ=train_JJ(sym_g, f_log_trash, times)
        # print(f"JJ : {time.time()-t0}")
        # f_log_JJ.write(f"{time.time()-t0}\n")
        # f_log_JJ.flush()
...
conda activate vv
cd Persistent_Message_Passing/Synthetic_graph/

# 0517
while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.3 --epoch 200; done
while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.4 --epoch 200; done
while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.5 --epoch 200; done
while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.6 --epoch 200; done
while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.7 --epoch 200; done
while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.8 --epoch 200; done


while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.35 --epoch 200; done
while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.45 --epoch 200; done
while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.55 --epoch 200; done
while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.65 --epoch 200; done
while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.75 --epoch 200; done
while sleep 1; do python TSBM_SGC2.py --L 0 --decay 0.85 --epoch 200; done


# node=2000
while sleep 1; do python TSBM_SGC2.py --L 3 --decay 0.8 --epoch 200; done [0.55 0.75]
while sleep 1; do python TSBM_SGC2.py --L 4 --decay 0.8 --epoch 200; done [0.4, 0.7]
while sleep 1; do python TSBM_SGC2.py --L 5 --decay 0.8 --epoch 200; done [0.4, 0.6]


while sleep 1; do python TSBM_SGC2.py --L 5 --decay 0.9 --epoch 200; done [0.4, 0.6]


while sleep 1; do python TSBM_SGC2.py --L 4 --decay 0.8 --epoch 200; done
'''