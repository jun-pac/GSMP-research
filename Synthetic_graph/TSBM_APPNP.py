...
from APPNPmodule import APPNP, APPNP_JJ, APPNP_PNY
...
def train_base(g, f_log):
    ...
    model = APPNP(features.shape[1], 16, 10).to(device)
...
def train_JJ(g, f_log, times):
    ...
    labels = g.ndata["label"]
    masks = g.ndata["train_mask"].type(torch.bool), g.ndata["val_mask"].type(torch.bool), g.ndata["test_mask"].type(torch.bool)
    model = APPNP_JJ(labels, times, args.num_time, args.num_label, args.split, features.shape[1], 16, 10).to(device)
    train(g, features, labels, masks, model, f_log)
    acc = evaluate(g, features, labels, masks[2], model)
    # print(f"{acc:.4f}", end=' ', flush=True)
    f_log.write(f"{acc:.4f}\n")
...
def train_PNY(g, f_log, relative_conn, times):
    ...
    labels = g.ndata["label"]
    masks = g.ndata["train_mask"].type(torch.bool), g.ndata["val_mask"].type(torch.bool), g.ndata["test_mask"].type(torch.bool)
    model = APPNP_PNY(labels, times, args.num_time, args.num_label, args.split, relative_conn, features.shape[1], 16, 10).to(device)
    train(g, features, labels, masks, model, f_log)
    acc = evaluate(g, features, labels, masks[2], model)
    # print(f"{acc:.4f}", end=' ', flush=True)
    f_log.write(f"{acc:.4f}\n")
...
if __name__ == "__main__":
    ...
    f_log_base=open('./txtlog/'+'APPNP_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_b','a')
    f_log_MMP=open('./txtlog/'+'APPNP_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_mmp','a')
    f_log_PMP=open('./txtlog/'+'APPNP_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pmp','a')
    f_log_JJ=open('./txtlog/'+'APPNP_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_jj','a')
    f_log_PNY=open('./txtlog/'+'APPNP_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pny','a')

    # f_log_base.write('APPNP_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_b'+'\n')
    # f_log_base.flush()
    # f_log_MMP.write('APPNP_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_mmp'+'\n')
    # f_log_MMP.flush()
    # f_log_PMP.write('APPNP_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pmp'+'\n')
    # f_log_PMP.flush()
    # f_log_JJ.write('APPNP_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_jj'+'\n')
    # f_log_JJ.flush()
    # f_log_PNY.write('APPNP_'+'L'+str(args.L)+'_'+'d'+str(args.decay)+'_'+str(args.K)+'_'+str(args.G)+'_pny'+'\n')
    # f_log_PNY.flush()    
...
while sleep 1; do python TSBM_APPNP.py --L 0 --decay 0.3 --epoch 100; done
while sleep 1; do python TSBM_APPNP.py --L 0 --decay 0.4 --epoch 100; done
while sleep 1; do python TSBM_APPNP.py --L 0 --decay 0.5 --epoch 100; done
while sleep 1; do python TSBM_APPNP.py --L 0 --decay 0.6 --epoch 100; done
while sleep 1; do python TSBM_APPNP.py --L 0 --decay 0.7 --epoch 100; done
while sleep 1; do python TSBM_APPNP.py --L 0 --decay 0.8 --epoch 100; done
