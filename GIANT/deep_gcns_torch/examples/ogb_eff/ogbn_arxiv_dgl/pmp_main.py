...
def pmp_preprocess(graph, paper_year):
    ...
    for i in range(len(src)):
        if abs(paper_year[src[i]]-paper_year[dst[i]])>min(2020-paper_year[dst[i]],paper_year[dst[i]]-1971): # pmp3 : 2000,
            # pmp4 : 1971~2020, n-rum=4
            newsrc.append(src[i])
            newdst.append(dst[i])
    for i in range(len(src)):
        newsrc.append(src[i])
...
def main():
    ...
    argparser.add_argument("--n-runs", type=int, default=4, help="running times")
