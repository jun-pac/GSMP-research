...
def load_data(dataset,args):
    ...
    print(f"train_idx.shape : {train_idx.shape}") # train_idx.shape : torch.Size([90941])
    print(f"train_idx[:30] : {train_idx[:30]}")
    # print(f"paper_year[train_idx[:30]] : {paper_year[train_idx[:30]]}")
    # print(f"paper_year[val_idx[:30]] : {paper_year[val_idx[:30]]}")
    # print(f"paper_year[test_idx[:30]] : {paper_year[test_idx[:30]]}")
    print(f"torch.min(paper_year[train_idx]), torch.max(paper_year[train_idx]) : {torch.min(paper_year[train_idx])}, {torch.max(paper_year[train_idx])}")
    print(f"torch.min(paper_year[val_idx]), torch.max(paper_year[val_idx]) : {torch.min(paper_year[val_idx])}, {torch.max(paper_year[val_idx])}")
    print(f"torch.min(paper_year[test_idx]), torch.max(paper_year[test_idx]) : {torch.min(paper_year[test_idx])}, {torch.max(paper_year[test_idx])}")

    n_train_idx=[]
    for i in range(len(train_idx)):
        if(paper_year[train_idx[i]]>=base_year):
            n_train_idx.append(train_idx[i])
    n_train_idx=torch.tensor(n_train_idx)
    print(f"torch.min(paper_year[n_train_idx]), torch.max(paper_year[n_train_idx]) : {torch.min(paper_year[n_train_idx])}, {torch.max(paper_year[n_train_idx])}")
    print(f"train_idx.dtype, n_train_idx.dtype, {train_idx.dtype}, {n_train_idx.dtype}")
    print(f"train_idx.shape, n_train_idx.shape, {train_idx.shape}, {n_train_idx.shape}")
    
    train_idx=n_train_idx
    # print(f"paper_year.shape : {paper_year.shape}") # paper_year.shape : torch.Size([169343])
    # print(f"graph.ndata.keys() : {graph.ndata.keys()}") # 'year' 'feat'
    # print(f"graph.ndata['year'].shape : {graph.ndata['year'].shape}") # graph.ndata['year'].shape : torch.Size([169343, 1])
    # print(f"graph.ndata['year'].dtype : {graph.ndata['year'].dtype}") # graph.ndata['year'].dtype : torch.int64
    # print(f"graph.edges() : {graph.edges()}") # graph.edges() : (tensor([104447,  15858, 107156,  ...,  45118,  45118,  45118]), tensor([ 13091,  47283,  69161,  ..., 162473, 162537,  72717]))
    # print(f"len(graph.edges()) : {len(graph.edges())}") # 2
    # print(f"graph.edges()[0].shape : {graph.edges()[0].shape}") # torch.Size([1166243])
    # print(f"graph.edges()[1].shape : {graph.edges()[1].shape}") # torch.Size([1166243])
    ...
    return graph, labels, train_idx, val_idx, test_idx, evaluator, paper_year


def pmp_preprocess(graph, paper_year):
    global n_node_feats
    ...
    for i in range(len(src)):
        if paper_year[src[i]]>=base_year and paper_year[dst[i]]>=base_year and abs(paper_year[src[i]]-paper_year[dst[i]])>min(2020-paper_year[dst[i]],paper_year[dst[i]]-base_year): # pmp3 : 2000,
            newsrc.append(src[i])
            newdst.append(dst[i])
    for i in range(len(src)):
        if paper_year[src[i]]>=base_year and paper_year[dst[i]]>=base_year:
            newsrc.append(src[i])
            newdst.append(dst[i])   
    src,dst=torch.tensor(newsrc),torch.tensor(newdst)
    ...
    # add self-loop
    print(f"Total edges before adding self-loop {new_g.number_of_edges()}") # 2864098
    new_g = new_g.remove_self_loop().add_self_loop()
    print(f"Total edges after adding self-loop {new_g.number_of_edges()}") # 3033441
...
def main():
    ...
    graph, labels, train_idx, val_idx, test_idx, evaluator, paper_year = load_data(dataset,args)
    
    graph = pmp_preprocess(graph,paper_year)
