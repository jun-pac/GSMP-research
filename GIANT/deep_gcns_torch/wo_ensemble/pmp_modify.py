...
def load_data(dataset,args):
    ...
    cnt=[0]*(2020-1971+1) # cnt[50]
    active=[True]*len(paper_year)

    for i in range(len(paper_year)):
        cnt[paper_year[i]-1971]+=1
    for i in range(len(cnt)):
        print(f"num year : {i+1971} : {cnt[i]}", flush=True)
    for i in range(len(paper_year)):
        if(paper_year[i]>=2010 and random.random()<5000/cnt[paper_year[i]-1971]):
            active[i]=True
        else:
            active[i]=False
    ...
    for idx in train_idx:
        if active[idx]:
            n_train_idx.append(idx)
    train_idx=torch.Tensor(n_train_idx)
    print(f"train_idx.dtype : {train_idx.dtype}")
    n_val_idx=[]
    for idx in val_idx:
        if active[idx]:
            n_val_idx.append(idx)
    val_idx=torch.Tensor(n_val_idx)
    n_test_idx=[]
    for idx in test_idx:
        if active[idx]:
            n_test_idx.append(idx)
    test_idx=torch.Tensor(n_test_idx)
    train_idx=train_idx.to(torch.long)
    val_idx=val_idx.to(torch.long)
    test_idx=test_idx.to(torch.long)
    print(f"train_idx.dtype : {train_idx.dtype}")
    print(f"val_idx.dtype : {val_idx.dtype}")
    ...
    return graph, labels, train_idx, val_idx, test_idx, evaluator, paper_year, active


def pmp_preprocess(graph, paper_year,active):
    global n_node_feats
    ...
    for i in range(len(src)):
        if(active[src[i]] and active[dst[i]] and abs(paper_year[src[i]]-paper_year[dst[i]])>min(2020-paper_year[dst[i]],paper_year[dst[i]]-2010)):
            newsrc.append(src[i])
            newdst.append(dst[i])
    for i in range(len(src)):
        if(active[src[i]] and active[dst[i]]):
            newsrc.append(src[i])
            newdst.append(dst[i])   
...
def main():
    ...
    graph, labels, train_idx, val_idx, test_idx, evaluator, paper_year,active = load_data(dataset,args)
    
    graph = pmp_preprocess(graph,paper_year,active)
