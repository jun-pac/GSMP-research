...
def load_data(dataset,args):
    ...
    # for i in range(1971,2020+1):
    #     print(f"year {i} : {cnt[i-1971]}")
...
def pmp_preprocess(graph, paper_year):
    ...
    src,dst=graph.edges()
    cnt=[[0]*(2020-1971+1) for _ in range(2020-1971+1)] # cnt[50][50]
    rel=[[0]*(2020-1971+1) for _ in range(2020-1971+1)] # rel[50][50]

    for i in range(len(src)):
        cnt[paper_year[src[i]]-1971][paper_year[dst[i]]-1971]+=1
    # for i in range(50):
    #     for j in range(50):
    #         print(f"{cnt[i][j]:.4f}", end=' ')
    #     print('',flush=True)
    ...
    for i in range(50):
        for j in range(50):
            sum[i]+=cnt[i][j]
        for j in range(50):
            rel[i][abs(i-j)]+=cnt[i][j]/max(1,sum[i])

    # print("relative...")
    # for i in range(50):
    #     print(f"year {i+1971} : ",end='')
    #     for j in range(50):
    #         print(f"{rel[i][j]:.4f}", end=' ')
    #     print('',flush=True)
    
    # print("max year")
    mul=[[0]*(2020-1971+1) for _ in range(2020-1971+1)]
    ...
    for i in range(50):
        ...
        for j in range(7):
            if(rel[i][j]>mx_val):
                mx_val=rel[i][j]
                mx_idx=j
        # print(mx_idx, mx_val,flush=True)
        if(mx_val<0.01):
            for j in range(50):
                mul[i][j]=1
            continue
        # print()
        for j in range(50):
            mul[i][j]=rel[48][j]/rel[48][mx_idx]*rel[i][mx_idx]/max(0.0001,rel[i][j])
        for j in range(50):
            mul[i][j]=int(mul[i][j]+0.4999)
    newsrc=[]
    newdst=[]
    
    for i in range(len(src)):
        for _ in range(mul[paper_year[src[i]]-1971][abs(paper_year[src[i]]-paper_year[dst[i]])]-1):
            newsrc.append(src[i])
            newdst.append(dst[i])   
    for i in range(len(src)):
        newsrc.append(src[i])
