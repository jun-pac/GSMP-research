...
def compute_edge_weights_chunk_rev(args):
    """Helper function for parallel edge weight computation (reversed approach)"""
...
def parallel_compute_rev(edge_weight, paper_year, row, col, num_nodes, num_workers=48):
    """
    Parallel computation of edge weights (reversed approach), exactly matching preprocess_GSMP_rev_papers100m.py
    """
    import numpy as np
    import multiprocessing
    ...
    with multiprocessing.Pool(processes=num_workers) as pool:
        results = pool.map(compute_edge_weights_chunk_rev, args_list)
    edge_weights = np.concatenate(results)
    edge_weight[:] = torch.from_numpy(edge_weights)


def compute_local_edge_weights_parallel_rev(graph, paper_year, num_workers=None):
    """
    Compute edge weights based on local time distribution using parallel processing (reversed approach).
    Exactly matching the approach in preprocess_GSMP_rev_papers100m.py
    """
    ...
    parallel_compute_rev(edge_weight, paper_year, row, col, num_nodes, num_workers)
    
    return edge_weight.float()


def compute_local_edge_weights_rev(graph, paper_year):
    """
    Compute edge weights based on local time distribution of neighbors (reversed approach).
    Sequential version for debugging and small graphs.
    Groups by destination nodes instead of source nodes.
    """
    src, dst = graph.edges()
    src = src.cpu().numpy()
    dst = dst.cpu().numpy()
    ...
    # Group edges by destination node (reversed approach)
    unique_dst, dst_indices = np.unique(dst, return_index=True)
    
    for i, dst_node in enumerate(unique_dst):
        # Find all edges to this destination node
        if i < len(unique_dst) - 1:
            edge_start = dst_indices[i]
            edge_end = dst_indices[i + 1]
        else:
            edge_start = dst_indices[i]
            edge_end = len(dst)
        
        # Get all source nodes pointing to this destination
        sources = src[edge_start:edge_end]
        if len(sources) == 0:
            continue
            
        # Get years of all source nodes
        source_years = paper_year[sources]
        
        # Count frequency of each year in the source neighborhood
        year_counts = np.bincount(source_years, minlength=2024)
        
        # Compute local weights for each edge
        local_weights = np.zeros(len(sources), dtype=np.float32)
        for idx, src_node in enumerate(sources):
            year = paper_year[src_node]
            # Weight is inversely proportional to the frequency of that year in the source neighborhood
            local_weights[idx] = 1.0 / (year_counts[year] if year_counts[year] > 0 else 1)
...
def heize_preprocess(graph, paper_year, delta_matrix, use_parallel=True, num_workers=None, cache_file="./gsmp_rev_edge_weight.pt"):
    global n_node_feats
    ...
    # Use local time distribution approach (reversed) instead of global delta_matrix
    print("Computing edge weights using local time distribution (reversed approach)...")
    ...
    if os.path.isfile(cache_file):
        ...
        print("Computing edge weights from scratch (reversed approach)...")
        if use_parallel:
            print(f"Using parallel computation with {num_workers or 'auto'} workers...")
            eweight = compute_local_edge_weights_parallel_rev(graph, paper_year, num_workers)
        else:
            print("Using sequential computation...")
            eweight = compute_local_edge_weights_rev(graph, paper_year)
...
def main():
    ...
    argparser.add_argument("--edge-weight-cache", type=str, default='./gsmp_rev_edge_weight.pt', help="Cache file for edge weights")
