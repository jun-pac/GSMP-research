...
from examples.ogb_eff.ogbn_arxiv_dgl.linear_model_weighted_rev import RevGAT
...
def heize_preprocess(graph, paper_year, delta_matrix):
    global n_node_feats
    ...
    src,dst=graph.edges()
    # print(f"graph.edata['w']: {graph.edata['w']}")
    eweight=[]
    for i in range(len(src)):
        yd=paper_year[dst[i]]-1971
        ys=paper_year[src[i]]-1971
        eweight.append(delta_matrix[test_time][abs(yd-ys)]/max(delta_matrix[yd][abs(yd-ys)],1))
        # eweight.append(1.0)
    ...
    print(f"new_g.edata['w']: {new_g.edata['w']}")
...
def main():
    ...
    argparser.add_argument("--kd_dir", type=str, default='./kd_linear_heize', help="kd path for pred")
