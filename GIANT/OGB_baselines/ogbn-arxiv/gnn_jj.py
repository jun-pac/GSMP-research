...
def main():
    ...
    for i in range(len(src)):
        if abs(paper_year[src[i]]-paper_year[dst[i]])>min(2020-paper_year[dst[i]],paper_year[dst[i]]-1971):
            newsrc.append(src[i])
            newdst.append(dst[i])   
    newsrc, newdst=torch.tensor(newsrc), torch.tensor(newdst)
    ...
    print(f"src.shape : {src.shape}") # src.shape : torch.Size([2864098])
    print(f"dst.shape : {dst.shape}") # dst.shape : torch.Size([2864098])
    
    data.adj_t=torch.sparse_coo_tensor((src,dst))
    coo_adj_t = data.adj_t.coo()    
    print(f"coo_adj_t[0].shape : {coo_adj_t[0].shape}") # row torch.Size([1166243]) -> torch.Size([2315598])
    print(f"coo_adj_t[1].shape : {coo_adj_t[1].shape}") # col torch.Size([1166243]) -> torch.Size([2315598])
