...
def main(args):
    ...
    checkpoint_dir = args.stage_checkpoint_dir.strip()
    checkpt_folder = checkpoint_dir if checkpoint_dir else f'./output/{args.dataset}/'
    ...
    checkpoint_prefix = args.stage_checkpoint_prefix.strip() or uuid.uuid4().hex
    checkpt_file = os.path.join(checkpt_folder, checkpoint_prefix)
    print(checkpt_file)

    gsmp_no_early_stop_stages = parse_impact_stages(
        args.gsmp_no_early_stop_stages or "none", len(args.stages))
    if args.impact_method == "gsmp" and gsmp_no_early_stop_stages:
        print(
            f"GSMP early stopping disabled for stages "
            f"{sorted(gsmp_no_early_stop_stages)}",
            flush=True,
        )
    if args.keep_stage_checkpoints:
        print(f"Keeping per-stage checkpoints under {checkpt_folder}", flush=True)

    for stage in range(args.start_stage, len(args.stages)):
        epochs = args.stages[stage]
        early_stop_enabled = not (
            args.impact_method == "gsmp" and stage in gsmp_no_early_stop_stages
        )
        ...
        for epoch in range(epochs):
            ...
            if epoch % args.eval_every == 0:
                ...
                if val_acc > best_val_acc:
                    ...
                    if early_stop_enabled and count >= args.patience:
                        break
    ...
    if not args.keep_stage_checkpoints:
        for stage in range(args.start_stage, len(args.stages)):
...
def parse_args(args=None):
    ...
    parser.add_argument("--gsmp-no-early-stop-stages", type=str, default="",
                        help="GSMP-only stage list/range where early stopping is disabled, e.g. 3")
    parser.add_argument("--keep-stage-checkpoints", action='store_true', default=False,
                        help="keep best model and raw prediction artifacts for every stage")
    parser.add_argument("--stage-checkpoint-dir", type=str, default="",
                        help="directory for kept per-stage checkpoint artifacts")
    parser.add_argument("--stage-checkpoint-prefix", type=str, default="",
                        help="filename prefix for per-stage checkpoints; defaults to a UUID")
