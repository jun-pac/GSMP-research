...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Ensemble best-validation logits.")
    parser.add_argument("--run_dirs", required=True, help="Space-separated component run dirs.")
    parser.add_argument("--seeds", default="42")
    parser.add_argument("--weights", default=None, help="Space-separated weights matching --run_dirs.")
    parser.add_argument("--data_root", default="../data")
    parser.add_argument("--output_dir", default="runs/ogbn_arxiv_simteg_tape_revgat_gsmp/csv")
...
def main() -> None:
    ...
    for seed in seeds:
        ...
        valid_acc = eval_acc(evaluator, y_true, y_pred, split_idx["valid"])
        test_acc = eval_acc(evaluator, y_true, y_pred, split_idx["test"])
        row = {
            ...
            "train_acc": train_acc,
            "valid_acc": valid_acc,
            "test_acc_at_best_valid": test_acc,
    ...
    summary = {
        ...
        "valid_acc_mean": mean([float(row["valid_acc"]) for row in rows]),
        "valid_acc_std": std([float(row["valid_acc"]) for row in rows]),
        "test_acc_at_best_valid_mean": mean([float(row["test_acc_at_best_valid"]) for row in rows]),
        "test_acc_at_best_valid_std": std([float(row["test_acc_at_best_valid"]) for row in rows]),
    ...
    print(
        f"ENSEMBLE_SUMMARY name={args.name} seeds={len(rows)} "
        f"valid={summary['valid_acc_mean']:.4f}+-{summary['valid_acc_std']:.4f} "
        f"test={summary['test_acc_at_best_valid_mean']:.4f}+-{summary['test_acc_at_best_valid_std']:.4f}",
        flush=True,
    )
