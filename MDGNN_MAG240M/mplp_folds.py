...
import socket
import logging
from features.config_feats import feats, model_feats
...
if __name__ == '__main__':
    ...
    model_name = os.path.basename(args.output_path.strip('/'))

    for model_featid in model_feats:
        if model_name in model_featid:
            feats_ids = model_featid[model_name]
            break

    assert feats_ids is not None

    feat_info = feats[[x[0] for x in feats_ids]]
    for feat_dim in feats_ids:
        if len(feat_dim) == 2:
            feat_info[feat_dim[0]] = (feat_info[feat_dim[0]][0], feat_info[feat_dim[0]][1], feat_dim[1])
    ...
    for k, (ti, vi) in enumerate(cv.split(valid_idx0)):
        ...
        with torch.no_grad():
            ...
            logger.info("the %2d-th fold valid accurate: %.4f" % (k,acc))
    ...
    for fpath in glob.glob(os.path.join(args.output_path, 'cv-*')):
